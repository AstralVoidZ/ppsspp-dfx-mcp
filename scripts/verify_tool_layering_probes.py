...
async def run() -> int:
    ...
    params = StdioServerParameters(
        command=PYTHON_EXE, args=["-m", "ppsspp_dfx_mcp"],
    ...
    async with stdio_client(params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            r = await session.call_tool(
                "ppsspp_session", {"action": "start", "iso_path": ISO_PATH})
            if _is_error(r):
            ...
            try:
                ...
                r = await session.call_tool(
                    ...
                    {"session_id": sid, "address": GAME_MODE_ADDR,
                     "access": "read", "timeout_s": 15.0,
                     "want_backtrace": True})
                s = _structured(r)
                print(f"[attempt1 read] hit={s.get('hit')} "
                ...
                if not s.get("hit"):
                    ...
                    task = asyncio.create_task(session.call_tool(
                        "ppsspp_trace_memory_access",
                        {"session_id": sid, "address": GAME_MODE_ADDR,
                         "access": "read_write", "timeout_s": 15.0,
                         "want_registers": True}))
                    await asyncio.sleep(2.0)
                    pr = await session.call_tool(
                        "ppsspp_press_button",
                        {"session_id": sid, "button": "cross",
                         "duration": 3})
                    print(f"[attempt2] press cross ok={not _is_error(pr)} "
                          f"(concurrent with trace — lock-free check)")
                    r = await task
                    s = _structured(r)
                    print(f"[attempt2 read_write] hit={s.get('hit')} "
                          f"bp_removed={s.get('bp_removed')} "
                ...
                ars = _structured(ar)
                raw_lines = [m.get("text", "")
                             for m in ars.get("matches", [])
                             if "[ws.raw]" in m.get("text", "")]
                print(f"A1-1 raw broadcast lines: {len(raw_lines)}")
                if raw_lines:
                    print(f"  sample: {raw_lines[0][:160]}")
                else:
                    failures.append(
                        "no [ws.raw] line in analyze_log(filter=cpu.stepping)")

                # A1-3: version fingerprint surfaced on session get.
                gr = await session.call_tool(
                    "ppsspp_session", {"action": "get", "session_id": sid})
                ...
                gs = _structured(await session.call_tool(
                    "ppsspp_query",
                    {"session_id": sid, "action": "game_state"}))
                print(f"final game_state.paused: "
                      f"{(gs.get('data') or {}).get('paused')}")
            finally:
