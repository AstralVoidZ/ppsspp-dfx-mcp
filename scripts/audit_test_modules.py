...
BASELINE = (PACKAGE_ROOT / "tests" / "unit"
            / "l2_mcp_contract" / "tool_surface_baseline.json")

HOLLOW_OK_MAX_RATIO = 0.20
# Session-independent tools may legitimately live in phase A only.
A_ONLY_OK = {
    "ppsspp_health", "ppsspp_list_addresses", "ppsspp_list_scripts",
...
def main() -> None:
    ...
    ap.add_argument("--check", action="store_true",
                    help="exit 1 when a gate trips (hollow-ok ratio, "
                         "zero-coverage tools, missing MCP surface)")
    args = ap.parse_args()

    all_s = list((*harness.PHASE_A, *harness.PHASE_B))
    by_tool: Counter[str] = Counter(s.tool for s in all_s)
    hollow = [s.id for s in all_s
              if s.expect == "ok" and s.validator is None]
    ratio = len(hollow) / len(all_s) if all_s else 1.0

    print(f"scenarios: {len(all_s)} "
          f"(A={len(harness.PHASE_A)}, B={len(harness.PHASE_B)})")
    print(f"hollow-ok: {len(hollow)}/{len(all_s)} = {ratio:.1%} "
          f"(gate ≤ {HOLLOW_OK_MAX_RATIO:.0%})")
    for sid in hollow:
        print(f"  hollow: {sid}")
    ...
    if BASELINE.exists():
        ...
        names = sorted(surface.keys()) if isinstance(surface, dict) else \
            sorted(t["name"] for t in surface)
        zero = [t for t in names if by_tool.get(t, 0) == 0]
        print(f"\ntool surface: {len(names)} static tools; "
              f"zero-coverage: {zero or 'none'}")

    surface_tools = set(by_tool)
    dynamic = sorted(t for t in surface_tools
                     if t.startswith("ppsspp_script_"))
    mcp_surface = {
        "prompts scenarios": by_tool.get("(prompts)", 0)
        + by_tool.get("(prompt)", 0),
        "resources scenarios": by_tool.get("(resources)", 0)
        + by_tool.get("(resource)", 0),
        "dynamic script scenarios": sum(by_tool.get(t, 0)
                                        for t in dynamic),
    }
    ...
    if args.check:
        ...
        if ratio > HOLLOW_OK_MAX_RATIO:
            problems.append(f"hollow-ok ratio {ratio:.1%} > "
                            f"{HOLLOW_OK_MAX_RATIO:.0%}")
        ...
        if mcp_surface["dynamic script scenarios"] < 2:
            problems.append("dynamic script tools not covered (need ≥2 "
                            "ppsspp_script_* scenarios)")
        for p in problems:
            print(f"GATE FAIL: {p}")
