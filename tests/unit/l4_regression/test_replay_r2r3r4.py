...
def _item(action: int, ts_us: int, u32: int = 0,
...
class TestParseReplayBlob:
    ...
    def test_corrupt_sidedata_length_rejected(self):
        ...
        blob = base64.b64encode(
            _item(MASK_SIDEDATA | 10, 1000, u32=100, payload=b"ab")
        ).decode("ascii")
...
def _make_client(saving: bool = False, executing: bool = False) -> AsyncMock:
    ...
    mock.replay_status.return_value = {
        "executing": executing, "saving": saving,
    }
...
def _write_ppr(tmp_path: Path, name: str, base64_data: str,
               base_rtc: int = 0) -> str:
    ppr_path = resolve_output_path("replays", name)
    ppr_path.write_text(json.dumps({
...
class TestExecuteLoadResponses:
    async def test_execute_carries_span_and_sequence(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        mock = _make_client()
        _patch_session_client(monkeypatch, mock)
        b64 = _b64(_item(BUTTONS, 5_000_000, u32=0x8))
        res = await replay_tool(
            session_id="s1", action="execute", version=1, base64_input=b64
        )
        data = res["data"]
    ...
    async def test_load_carries_span(self, tmp_path, monkeypatch):
        ...
        res = await replay_tool(
            session_id="s1", action="load", file_path=name
        )
        data = res["data"]
...
class TestAutoAbort:
    async def test_execute_aborts_busy_replay_first(
        self, monkeypatch: pytest.MonkeyPatch
    ):
    ...
    async def test_load_aborts_saving_replay_first(
        ...
        _isolate_output_dir(tmp_path, monkeypatch)
        name = _write_ppr(
            tmp_path, "busy.ppr", _b64(_item(BUTTONS, 1_000_000, u32=0x8))
        )
        await replay_tool(session_id="s1", action="load", file_path=name)
        mock.replay_abort.assert_awaited_once()

    async def test_idle_replay_not_aborted(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        mock = _make_client()
        _patch_session_client(monkeypatch, mock)
...
class TestRestoreRtc:
    async def test_default_off_does_not_touch_clock(self, tmp_path, monkeypatch):
        ...
        name = _write_ppr(
            tmp_path, "nortc.ppr",
            _b64(_item(BUTTONS, 1_000_000, u32=0x8)), base_rtc=1_788_891_921,
        )
    ...
    async def test_true_restores_even_zero_base_rtc(
        self, tmp_path, monkeypatch
    ):
        ...
        name = _write_ppr(
            tmp_path, "epoch0.ppr",
            _b64(_item(BUTTONS, 1_000_000, u32=0x8)), base_rtc=0,
        )
        res = await replay_tool(
            session_id="s1", action="load", file_path=name, restore_rtc=True
        )
