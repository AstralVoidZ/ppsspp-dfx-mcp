...
async def test_w7_call_send_failure_does_not_leak_pending():
    ...
    with patch(
        ...
        assert t._pending == {}, (
            "W7: a failed send must not strand a future in _pending"
        )
        await t.close()
    ...
    async def recv(self) -> str:
        ...
        for _ in range(100):
            ...
            if self.sent:
                ...
                return json.dumps(
                    {"event": msg.get("event", "?"), "ticket": msg["ticket"]}
                )
...
async def test_w7_recv_loop_survives_malformed_message():
    ...
    with patch(
        ...
        assert t.is_connected(), (
            "W7: the recv loop must still be alive after a bad frame"
        )
        await t.close()
...
@pytest.mark.asyncio
async def test_w8_gc_pops_and_closes_transport_and_observer(
    ...
    with patch.object(proc, "is_pid_alive", return_value=False), patch.object(
        sm, "_kill_stale_pid_safe"
    ) as kill_mock:
        stopped = await mgr.gc_idle_sessions()

    assert "sess-gone" in stopped
    assert "sess-gone" not in mgr._transports, (
        "W8: GC must pop the session transport"
    )
    assert "sess-gone" not in mgr._observers, (
        "W8: GC must pop the session observer"
    )
...
def test_w9_pid_name_check_refuses_non_ppsspp():
    ...
    with patch.object(sm, "_pid_name_is_ppsspp", return_value=False), patch.object(
        sm, "_force_kill_pid"
    ) as kill_mock:
...
def test_w9_pid_name_check_kills_when_unknown():
    ...
    with patch.object(sm, "_pid_name_is_ppsspp", return_value=None), patch.object(
        sm, "_force_kill_pid"
    ) as kill_mock:
        sm._kill_stale_pid_safe(4242)
...
async def test_w9_stop_session_refuses_reused_pid(tmp_path, monkeypatch):
    ...
    mgr = sm.SessionManager()
    with patch.object(proc, "is_pid_alive", return_value=True), patch.object(
        sm, "_pid_name_is_ppsspp", return_value=False
