...
SRC_TOOLS = (Path(__file__).resolve().parents[3] / "src" / "ppsspp_dfx_mcp"
             / "tools").resolve()
...
HOLDING_LOCK: frozenset[str] = frozenset({
    ...
    "ppsspp_query", "ppsspp_get_pc", "ppsspp_write_register",
    "ppsspp_evaluate", "ppsspp_search_disasm", "ppsspp_assemble",
    "ppsspp_breakpoint", "ppsspp_step", "ppsspp_smoke_test",
    "ppsspp_state_observer", "ppsspp_batch_step", "ppsspp_replay",
...
_SESSION_OPENERS = {
    "session_client", "session_capture", "session_client_with_transport",
}
...
def test_partial_hold_tools_open_a_session_context():
    ...
    tools = _tool_functions()
    liars = sorted(
        name for name in PARTIAL_HOLD_LOCK
        if name in tools and not _calls_session_opener(tools[name])
    )
    assert not liars, (
...
def test_non_holders_do_not_open_a_session_context():
    tools = _tool_functions()
    violators = sorted(
        name for name in NOT_HOLDING_LOCK
        if name in tools and _calls_session_opener(tools[name])
    )
