...
|---|---|---|
| 连接 | `version` / `cpu.status` | `session`(get/smoke)、`smoke_test`、`gpu_stats` |
| game | `game.status` `game.start` `game.pause` `game.resume` `game.reset` `game.quit` | `session`（start/stop/get/wait_ready）、`step`(pause/resume/reset)、`batch_step` |
| cpu 走查 | `cpu.stepping` `cpu.stepInto` `cpu.stepOver` `cpu.stepOut` `cpu.resume` `cpu.nextHLE` `cpu.runUntil` | `step`（into/over/out/resume/next_hle/run_until）、`frame_snapshot`、`batch_step` |
...
| cpu 断点 | `cpu.breakpoint.add/list/remove/update` | `breakpoint`（set/remove/list/update）、`wait_breakpoint`、`trace_memory_access` |
...
- `trace_memory_access` = 内存断点武装 → 命中捕获（寄存器/回溯）→ 清理 → 恢复
- `batch_step` = press/wait/state_probe/screenshot 的后台任务编排
  （detached task，独立于 MCP 工具调用超时）
